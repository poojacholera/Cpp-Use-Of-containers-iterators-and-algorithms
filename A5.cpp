...
int CountStringFreeFun(const string & str, const int & n) {
    return str.length() == n;
}
int main()
{
...
int testCountStringFreeFun(const std::vector<string> & vecstr, int n) {
    ...
    result = count_if(vecstr.begin(), vecstr.end(), std::bind(CountStringFreeFun,_1,n));
...
int testCountStringFunctor(const std::vector<string> &vecstr, int n) {
    ...
    class StringFunctor {
    public:
        bool operator() (const std::string &s1, int n) const
        {
            return s1.length() == n;
        }
    };
    ...
    auto strLenComp=std::bind(StringFunctor(),_1,n);
    result = count_if(vecstr.begin(), vecstr.end(),strLenComp);
