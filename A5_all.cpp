...
string temp ;
    ...
    std::remove_copy_if(phrase.c_str(), phrase.c_str()+phraseSize,insert_it, [](char c) {return !std::isalpha(c,std::locale());});
    ...
    std::transform(temp.begin(),temp.end(),temp.begin(),[](char ch){ return std::toupper(ch);});
...
void charFrequency() {
    ...
    string input;
    //reads a string of any number of characters from the keyboard(except white space)
    cin >> std::skipws;
    while (cin){
        string str;
        cin>>str;
...
cout<<"^D"<<endl;// output is for the Ubuntu machine that does not print ^D
//cout<<input;
std::map<char,int> index;
std::for_each(input.begin(),input.end(),[& index](const char& c){index[c]++;});//uses lambda expression
   //1. Write a function charFrequency() that reads a string of any number of characters from the keyboard,
   // storing and counting the frequencies of each input character (except whitespace).
    //2. Your code must use a map<char, int> container to store the characters (keys) and their frequency counts (values) as pair<char, int> objects.
    // Do not use any loops;instead, use the for_each algorithm to extract characters from the input stream and
    // reflect the characters and their frequency counts into the map.

       //     Hint: although the task at hand here might seem very involved, reading input characters and counting their frequencies
       //     can be implemented using just one statement! Simply, implement the callable parameter of the for_each algorithm as a lambda expression,
       //     capturing your map object into your lambda by reference.
    for(auto& pair: index) {
...
//    int largest_element = *start; //also let, first element is the biggest one
    ...
    else if(std::adjacent_find(start, finish, std::not_equal_to<>()) == finish)
    {
